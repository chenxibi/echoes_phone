...
// --- 辅助函数：获取所有分组（按存储的顺序）---
const getGroups = (data, groupOrder) => {
  const groups = Array.from(
    new Set(data.map((item) => item.group || "未分组")),
  );
  if (groupOrder && groupOrder.length > 0) {
    // 按 groupOrder 排序，未在 order 中的排最后
    const orderSet = new Set(groupOrder);
    groups.sort((a, b) => {
      const ai = orderSet.has(a) ? groupOrder.indexOf(a) : 999;
      const bi = orderSet.has(b) ? groupOrder.indexOf(b) : 999;
      return ai - bi;
    });
  } else {
    groups.sort((a, b) =>
...
const WorldBook = ({
  ...
  // 拖拽排序 —— 分组顺序
  const [groupOrder, setGroupOrder] = useStickyState([], "echoes_wb_group_order");
  ...
  const handleGroupReorder = (fromGroup, toGroup) => {
    const groups = getGroups(worldBook, groupOrder);
    const fromIdx = groups.indexOf(fromGroup);
    const toIdx = groups.indexOf(toGroup);
  ...
  return (
    <AppWindow isOpen={isOpen} title="世界书" onClose={onClose}>
      <div className="space-y-6 pt-4 pb-20">
        ...
        {/* 分组列表 */}
        {getGroups(worldBook, groupOrder).map((group, idx, groups) => (
          <WorldBookGroup
            key={group}
            group={group}
            groupIdx={idx}
...
const WorldBookGroup = ({
  ...
  return (
    <div className={`bg-white/50 border border-gray-100 rounded-xl overflow-hidden mb-4 shadow-sm transition-all ${isDraggingGroup ? "opacity-40 scale-95" : ""}`}>
      <div
        ...
        onDrop={(e) => {
          ...
          if (from && from !== group) onGroupReorder(from, group);
        }}
      >
